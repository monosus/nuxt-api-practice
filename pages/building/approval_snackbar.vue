<template>
  <div>
    <LayoutDefault
      pageTitle="A-1000002 / 作業届"
      labelText="承認済み"
      labelStyle="purple"
      :current="2"
    >
      <template #headerButton>
        <ButtonText text="差し戻し画面へ" isDisabled />
        <ButtonText text="承認画面へ" isDisabled />
      </template>
      ...
      <SnackBar
        text="申請を承認しました"
        :isShow="isSnackBarShow"
        @close="isSnackBarShow = false"
      />
...
export default {
  setup() {
    useHead({ title: 'ビル申請管理_ビル申請_承認完了' })
    const isSnackBarShow = ref(true)
    const comments = [
      {
        text: '申請内容を確認し、承認しました。危険物に該当する物品の使用予定がありそうですので、続いて「危険物使用（持込）許可願」の申請もお願いします。\r\nその際、親申請No.として「A-1000002」を入力ください。',
        date: '2022/12/21',
        author: 'ビル管理会社',
      },
      {
